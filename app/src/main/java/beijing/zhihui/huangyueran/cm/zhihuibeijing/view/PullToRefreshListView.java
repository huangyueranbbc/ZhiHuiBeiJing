...
/**
 * 自定义下拉刷新控件 ListView
 * Created by huangyueran on 2016/12/9.
 */
public class PullToRefreshListView extends ListView implements AbsListView.OnScrollListener {
    ...
    public boolean onTouchEvent(MotionEvent ev) {
        switch (ev.getAction()) {
            ...
            case MotionEvent.ACTION_MOVE:
                // 当用户按住头条新闻的viewpager进行下拉时，ACTION_DOWN会被viewpager消费掉，导致startY没有被及时赋值，需要重新进行获取
                if (startY == -1) {
                    startY = (int) ev.getY();
                }
                ...
                if (dy > 0 && firstVisiblePosition == 0) {
                    int paddint = dy - mHeaderViewHeight;   //负高度
                    mHeaderView.setPadding(0, paddint, 0, 0);

                    if (paddint > 0 && mCurrentState != STATE_RELEASE_TO_REFRESH) {
                        // 改为松开刷新状态
                        mCurrentState = STATE_RELEASE_TO_REFRESH;
                        refreshState();
                    } else if (paddint < 0 && mCurrentState != STATE_PULL_TO_REFRESH) {
                        // 改为下拉刷新
    ...
    private void refreshState() {
        switch (mCurrentState) {
            case STATE_PULL_TO_REFRESH:
                ...
                pbProgress.setVisibility(View.INVISIBLE); //显示下拉箭头
                ivArrow.setVisibility(View.VISIBLE); //隐藏进度条
            ...
            case STATE_REFRESHING:
                ...
                ivArrow.clearAnimation(); //清楚箭头动画
                pbProgress.setVisibility(View.VISIBLE); //显示进度条
                ivArrow.setVisibility(View.INVISIBLE); //隐藏下拉箭头
                break;
    ...
    public void onScrollStateChanged(AbsListView view, int scrollState) {
        if (scrollState == SCROLL_STATE_IDLE) { // 空闲状态
            ...
            if (lastVisiblePosition == getCount() - 1 && !isLoadMore) { //如果是最后一个item并且没有正在加载更多
                // 到底了
                System.out.println("加载更多...");
                isLoadMore=true;

                mFooterView.setPadding(0, 0, 0, 0);// 显示加载更多的脚布局

                setSelection(getCount() - 1); // 讲ListView的位置显示在最后一个item上
